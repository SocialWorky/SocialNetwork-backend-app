...
export class FriendsService {
  ...
  async sendFriendRequest(senderId: string, receiverId: string): Promise<{ message: string; friendshipId: string }> {
  const sender = await this.userRepository.findOne({ where: { _id: senderId } });
  const receiver = await this.userRepository.findOne({ where: { _id: receiverId } });
  ...
  const existingRequest = await this.friendshipRepository.findOne({
    where:
      { requester: sender} && { receiver: receiver} && { status: Status.PENDING},
  }); 

  const existingFriendship = await this.friendshipRepository.findOne({
    where: 
      { requester: sender} && { receiver: receiver} && { status: Status.ACCEPTED},
  });

  if (existingRequest) {
     if (existingRequest.isBlocked) {
       throw new ConflictException('Cannot send friend request, one of the users is blocked.');
     }
  ...
  return { message: 'Friend request sent successfully.', friendshipId: savedFriendship.id };
}

  async acceptFriendRequest(friendshipId: string): Promise<void> {
    const friendship = await this.friendshipRepository.findOne({
      where: 
        { id: friendshipId } && { status: Status.PENDING },
      relations: ['requester', 'receiver'],
    });
